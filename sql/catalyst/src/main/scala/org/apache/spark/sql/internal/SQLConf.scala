...
object SQLConf {
  ...
  val PYTHON_OFFSET_PUSHDOWN_ENABLED = buildConf("spark.sql.python.offsetPushdown.enabled")
    .internal()
...
class SQLConf extends Serializable with Logging with SqlApiConf {
  ...
  def pythonOffsetPushDown: Boolean = getConf(PYTHON_OFFSET_PUSHDOWN_ENABLED)
