...
@ExpressionDescription(
  usage = """
    _FUNC_(time_expr) - Returns the second component of the given time.
  """,
  examples = """
    Examples:
...
case class SecondsOfTimeWithFraction(child: Expression)
  ...
  override def replacement: Expression = {
    ...
    StaticInvoke(
      ...
      DecimalType(2 + timePrecision, timePrecision),
      "getSecondsOfTimeWithFraction",
      Seq(child, Literal(timePrecision)),
      Seq(child.dataType, IntegerType))
  }
  private lazy val timePrecision: Int = child.dataType match {
    case t: TimeType => t.precision
    case _ => 6
  }
  override def inputTypes: Seq[AbstractDataType] = Seq(TimeType(timePrecision))
  ...
  override def build(name: String, expressions: Seq[Expression]): Expression = {
    if (expressions.isEmpty) {
      ...
      child.dataType match {
        case _: TimeType =>
          SecondsOfTimeWithFraction(child)
        case _ =>
          Second(child)
      }
    }
