...
case class CollectSet(
  ...
  // distinct. In that case we store a wrapper in the HashSet whose equality/hashing use a
  // collation-aware key, while keeping the original first-seen value for output. Binary-stable
  // inputs (the common case, incl. MapType which is rejected in checkInputDataTypes) skip all of
  // this and stay on the fast path below.
  @transient private lazy val needsCollationKey: Boolean =
    !UnsafeRowUtils.isBinaryStable(child.dataType)

  // Projects a value to a collation- and float-normalized UnsafeRow and returns its bytes as the
  // dedup key. injectCollationKey replaces non-binary collated strings (including those nested in
  // structs/arrays) with their collation key bytes, and NormalizeFloatingNumbers collapses
  // -0.0/0.0 and NaNs so any nested floats dedup consistently, mirroring complexNormalizer.
