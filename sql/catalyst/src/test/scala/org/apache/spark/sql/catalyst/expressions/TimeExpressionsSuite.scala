...
class TimeExpressionsSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("Second with TIME type") {
    ...
    assert(SecondsOfTimeWithFraction(Literal.create(null, TimeType())).resolved)
    assert(
      SecondsOfTimeWithFraction(
        Cast(Literal(time), TimeType(TimeType.MICROS_PRECISION))).resolved)
    assert(
      SecondsOfTimeWithFraction(
        Literal.create(time, TimeType(TimeType.MIN_PRECISION + 3))).resolved)
    Seq(
      0 -> 15.toDouble,
      1 -> 15.9,
      2 -> 15.98,
      ...
      checkEvaluation(
        SecondsOfTimeWithFraction(Literal.create(time, TimeType(precision))),
        BigDecimal(expected))
    }
    // Verify NULL handling
    checkEvaluation(
