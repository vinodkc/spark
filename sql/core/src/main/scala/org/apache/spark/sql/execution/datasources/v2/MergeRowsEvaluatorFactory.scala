...
import org.apache.spark.sql.catalyst.expressions.{Attribute, BasePredicate, Expression, Projection, UnsafeProjection}
...
class MergeRowsEvaluatorFactory(
  ...
  override def createEvaluator(): PartitionEvaluator[InternalRow, InternalRow] =
    new MergeRowsEvaluator

  private class MergeRowsEvaluator extends PartitionEvaluator[InternalRow, InternalRow] {
    ...
    private def createProjection(exprs: Seq[Expression]): UnsafeProjection = {
      UnsafeProjection.create(exprs, childOutput)
    }

    private def createPredicate(expr: Expression): BasePredicate = {
      GeneratePredicate.generate(expr, childOutput)
    }
    ...
    override def eval(
      ...
      val cardinalityValidator = if (checkCardinality) {
        val rowIdOrdinal = childOutput.indexWhere(attr => resolver(attr.name, ROW_ID))
