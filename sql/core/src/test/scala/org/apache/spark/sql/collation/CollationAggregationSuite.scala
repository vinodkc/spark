...
class CollationAggregationSuite
  ...
  test("collect_union is collation-aware for collated strings nested at depth >= 2") {
    // The element is an array of structs, nesting the collated string two levels deep
    // (element array -> struct -> string), exercising the injectCollationKey recursion below
    // the top-level struct/array element cases above.
    val df = spark.sql(
      "SELECT array(array(named_struct('s', CAST(c AS STRING COLLATE UTF8_LCASE)))) AS arr " +
        "FROM VALUES ('foo'), ('FOO'), ('bar') AS t(c)")

    checkAnswer(df.select(size(collect_union($"arr"))), Seq(Row(2)))
    checkAnswer(
      df.select(array_sort(transform(collect_union($"arr"), x => lower(x(0)("s"))))),
      Seq(Row(Seq("bar", "foo"))))
  }

  test("collect_union dedups elements on a space-trimming collation (UTF8_LCASE_RTRIM)") {
    val df = spark.sql(
      "SELECT array(CAST(c AS STRING COLLATE UTF8_LCASE_RTRIM)) AS arr " +
        "FROM VALUES ('foo '), ('foo'), ('FOO'), ('bar'), ('BAR  ') AS t(c)")
    ...
    checkAnswer(df.select(size(collect_union($"arr"))), Seq(Row(2)))
    checkAnswer(
      df.select(array_sort(transform(collect_union($"arr"), x => trim(lower(x))))),
      Seq(Row(Seq("bar", "foo"))))
  }
