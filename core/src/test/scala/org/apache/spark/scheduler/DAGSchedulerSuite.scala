...
class DAGSchedulerSuite extends SparkFunSuite with TempLocalSparkContext with TimeLimits {
  ...
  test("cache location preferences w/ dependency") {
    ...
    val finalRdd = new MyRDD(sc, 1, List(new OneToOneDependency(baseRdd)))
    cacheLocations(baseRdd.longId -> 0) =
  ...
  test("regression test for getCacheLocs") {
    ...
    cacheLocations(rdd.longId -> 0) =
      Seq(makeBlockManagerId("hostA"), makeBlockManagerId("hostB"))
    cacheLocations(rdd.longId -> 1) =
      Seq(makeBlockManagerId("hostB"), makeBlockManagerId("hostC"))
    cacheLocations(rdd.longId -> 2) =
  ...
  test("cached post-shuffle") {
    ...
    val finalRdd = new MyRDD(sc, 1, List(shuffleDepTwo), tracker = mapOutputTracker)
    submit(finalRdd, Array(0))
    cacheLocations(shuffleTwoRdd.longId -> 0) = Seq(makeBlockManagerId("hostD"))
    cacheLocations(shuffleTwoRdd.longId -> 1) = Seq(makeBlockManagerId("hostC"))
    // complete stage 0
    completeShuffleMapStageSuccessfully(0, 0, 2)
    // complete stage 1
    completeShuffleMapStageSuccessfully(1, 0, 1)
  ...
  test("SPARK-53564: Resubmit missing parent when failed to get cache locations") {
    ...
    cacheLocations(shuffleMapRdd1.longId -> 0) = Seq(makeBlockManagerId("hostA"))
    cacheLocations(shuffleMapRdd1.longId -> 1) = Seq(makeBlockManagerId("hostB"))
